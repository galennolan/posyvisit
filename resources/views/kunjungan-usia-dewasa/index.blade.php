<!-- resources/views/kunjungan-usia-dewasa/index.blade.php -->

<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{ __('Data Kunjungan Usia Dewasa') }}
        </h2>
        <nav class="breadcrumb">
            <ol class="list-reset flex text-sm">
                <li><a href="/dashboard" class="text-blue-600 hover:text-blue-800">Home</a></li>
                <li><span class="mx-2">/</span></li>
                <li class="text-blue-600 font-semibold"><a href="{{ route('kunjungan-usia-dewasa.index') }}" class="text-blue-600 hover:text-blue-800">Kunjungan Usia Dewasa</a></li>
            </ol>
        </nav>
    </x-slot>
    ...
    <div class="py-12" x-data="modalHandler()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-lg p-6">
                ...
                <!-- Form Pencarian -->
                <form method="GET" action="{{ route('kunjungan-usia-dewasa.index') }}" class="mb-4">
                    <div class="flex">
                        <input type="text" name="search" value="{{ request('search') }}" class="border border-gray-300 rounded-md py-2 px-4" placeholder="Cari Nama...">
                ...
                <div class="table-responsive">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($kunjunganUsiaDewasa as $index => $kunjungan)
                                <tr class="hover:bg-gray-50 transition duration-200">
                                    <td class="px-4 py-2 text-sm text-gray-500">{{ $kunjunganUsiaDewasa->firstItem() + $index }}</td>
                                    <td class="px-4 py-2 text-sm text-gray-700">{{ $kunjungan->anggotaKeluarga->nama_lengkap}}</td>
                                    <td class="px-4 py-2 text-sm text-gray-700">{{ $kunjungan->umur }} tahun</td>
                                    <td class="px-4 py-2 text-sm text-gray-700">{{ \Carbon\Carbon::parse($kunjungan->tanggal_kunjungan)->format('d/m/Y') }}</td>
                                    <td class="px-4 py-2 text-sm text-gray-700">{{ $kunjungan->suhu_tubuh }}</td>
                                    <td class="px-4 py-2 flex space-x-2">
                                        <!-- Tombol Lihat -->
                                        <button class="text-blue-500 hover:text-blue-700" @click="openModal({{ $kunjungan->id }})">
                                            Lihat
                                        </button>
                                        <a href="{{ route('kunjungan-usia-dewasa.edit', $kunjungan->id) }}" class="text-yellow-500 hover:text-yellow-700">Edit</a>
                                        <form action="{{ route('kunjungan-usia-dewasa.destroy', $kunjungan->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <!-- Link Pagination -->
                    <div class="mt-4">
                        {{ $kunjunganUsiaDewasa->appends(request()->input())->links() }}
                    </div>
                    ...
                    >
                        <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full relative">
                            ...
                            <!-- Konten Modal -->
                            <h3 class="text-lg font-semibold text-gray-700 mb-4">Detail Kunjungan Usia Dewasa</h3>
                        ...
                        function modalHandler() {
                            return {
                                ...
                                openModal(id) {
                                    ...
                                    fetch(`/kunjungan-usia-dewasa/${id}`)
