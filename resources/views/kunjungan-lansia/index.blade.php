<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{ __('Data Kunjungan lansia') }}
        </h2>
        <nav class="breadcrumb">
            <ol class="list-reset flex text-sm">
                <li><a href="/dashboard" class="text-blue-600 hover:text-blue-800">Home</a></li>
                <li><span class="mx-2">/</span></li>
                <li class="text-blue-600 font-semibold"><a href="{{ route('kunjungan-lansia.index') }}" class="text-blue-600 hover:text-blue-800">Kunjungan lansia</a></li>
            </ol>
        </nav>
    </x-slot>
    ...
    <div class="py-12" x-data="modalHandler()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-lg p-6">
                <h3 class="text-lg font-semibold text-gray-700 mb-4">Daftar Kunjungan lansia</h3>
                
                <!-- Form Pencarian -->
                <form method="GET" action="{{ route('kunjungan-lansia.index') }}" class="mb-4">
                    <div class="flex">
                ...
                <div class="table-responsive">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($kunjunganLansia as $index => $kunjungan)
                                <tr class="hover:bg-gray-50 transition duration-200">
                                    <td class="px-4 py-2 text-sm text-gray-500">{{ $kunjunganLansia->firstItem() + $index }}</td>
                                    ...
                                    <td class="px-4 py-2 flex space-x-2">
                                        ...
                                        <a href="{{ route('kunjungan-lansia.edit', $kunjungan->id) }}" class="text-yellow-500 hover:text-yellow-700">Edit</a>
                                        <form action="{{ route('kunjungan-lansia.destroy', $kunjungan->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <!-- Link Pagination -->
                    <div class="mt-4">
                        {{ $kunjunganLansia->appends(request()->input())->links() }}
                    </div>
                    ...
                    >
                        <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full relative">
                            ...
                            <!-- Konten Modal -->
                            <h3 class="text-lg font-semibold text-gray-700 mb-4">Detail Kunjungan lansia</h3>
